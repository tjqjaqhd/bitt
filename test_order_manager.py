...
def test_order_manager():
    ...
    try:
        ...
        # 테스트 주문 2: SOL 지정가 매도 (보유하지 않은 수량으로 검증 실패 예상)
        sell_request = OrderRequest(
            symbol="SOL_KRW",
            side=OrderSide.SELL,
            order_type=OrderType.LIMIT,
            quantity=Decimal('1.0'),  # 보유량보다 많은 수량
        ...
        # 주문 취소 테스트 (있는 경우)
        if pending_orders:
            cancel_order = pending_orders[0]
            print(f"\n❌ 주문 취소 테스트: {cancel_order.client_order_id}")
            success = order_manager.cancel_order(cancel_order.client_order_id)
        ...
        print(f"❌ 모듈 임포트 오류: {e}")
        print("💡 필요한 모듈들이 올바르게 구현되었는지 확인해주세요.")
