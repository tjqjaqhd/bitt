...
- **데이터 처리**: `pandas`, `numpy`
- **기술적 지표**: `ta` 또는 `TA-Lib`
- **거래소 연동**: 빗썸 API 2.0 (JWT 인증)
- **JWT 인증**: `PyJWT==2.8.0` (빗썸 API 2.0 필수)
...
2. **JWT 인증**: 빗썸 API 2.0은 JWT (HS256) 인증 방식 사용
3. **인증 헤더**: `Authorization: Bearer {jwt_token}` 형식
4. **Rate Limit**: 빗썸 API 호출 제한 준수 (초당 요청 수 제한)
5. **종목 코드 형식**: KRW 마켓은 `KRW-BTC` 형식 사용
6. **수수료**: 거래 수수료 0.25% 기준 계산 (VIP 등급별 차등)
7. **API 2.0 엔드포인트**: `/v1/accounts` 등 새로운 엔드포인트 사용
...
### 📄 문서 동기화 필수 규칙
**모든 작업 완료 후 반드시 문서 동기화를 수행해야 합니다:**
1. **WSL → Windows 동기화**: `cp -r ~/projects/bithumb-trading-system/* /mnt/c/Users/SEOBEOMBONG/gogo/`
2. **문서 업데이트**: 체크리스트, 진행상황, 보고서 등 모든 문서 최신화
3. **Git 커밋**: 변경사항 GitHub에 반영
4. **동기화 확인**: 양쪽 디렉토리 일치 확인

⚠️ **이 규칙을 지키지 않으면 작업 진행상황이 손실될 수 있습니다.**

## 개발 진행 상황

### ✅ 완료된 Phase
1. **Phase 0**: 개발환경 구축 (100% 완료)
2. **Phase 1**: 빗썸 API 2.0 연동 및 종목 동기화 (100% 완료)
3. **Phase 2**: 데이터베이스 설계 및 구현 (100% 완료)
4. **Phase 3**: 전략 엔진 구현 (100% 완료)
5. **Phase 4**: 주문 실행 시스템 (100% 완료)
6. **Phase 5**: 백테스트 시스템 (100% 완료)
7. **Phase 6**: 대시보드 구현 (100% 완료)

### 🔄 현재 진행
- **Phase 7**: 테스트 및 안정화 (다음 단계)

### 📋 다음 단계
8. **Phase 8**: 실전 배포

### 🎯 주요 성과
- 빗썸 API 2.0 JWT 인증 완전 구현
- 실제 계좌 연동 성공 (KRW 147,059원, 8종 코인 보유)
- 414개 KRW 마켓 실시간 데이터 연동
- EMA + RSI + ATR 전략 엔진 완성
- 주문 실행 시스템 구현 완료
- 백테스트 및 성과 분석 시스템 완성
